	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals(CHANGE_CONFIG_ACTION)) {
			...
			if(intent.getExtras() != null) {
				...
				if(intent.hasExtra("addssid")) {
					String lssid = intent.getStringExtra("addssid");
					if(lssid!=null) {
						List<String> listssid = new LinkedList<>(Arrays.asList(setting.getString("ssid", "").split(" , ")));
						for(String s:lssid.split(",")){
							if(listssid.contains(s.trim())) listssid.add(s.trim());
						}
						lssid = TextUtils.join(" , ", listssid);
						editor.putString("ssid", lssid);
						editor.commit();
				...
				if(intent.hasExtra("delssid")) {
					String lssid = intent.getStringExtra("delssid");
					if(lssid!=null) {
						List<String> listssid = new LinkedList<>(Arrays.asList(setting.getString("ssid", "").split(" , ")));
						for(String s:lssid.split(",")){
							if(listssid.contains(s.trim())) listssid.remove(s.trim());
						}
						lssid = TextUtils.join(" , ", listssid);
						editor.putString("ssid", lssid);
						editor.commit();
				...
				if(intent.hasExtra("setexcludedssid")) {
					String lssid = intent.getStringExtra("setexcludedssid");
					if(lssid!=null) {
						List<String> listssid = new ArrayList<>();
						for(String s:lssid.split(",")){
							listssid.add(s.trim());
						...
						lssid = TextUtils.join(" , ", listssid);
						editor.putString("excludedSsid", lssid);
						editor.commit();
					}
				}
				if(intent.hasExtra("addexcludedssid")) {
					String lssid = intent.getStringExtra("addexcludedssid");
					if(lssid!=null) {
						List<String> listssid = new LinkedList<>(Arrays.asList(setting.getString("excludedSsid", "").split(" , ")));
						for(String s:lssid.split(",")){
							if(listssid.contains(s.trim())) listssid.add(s.trim());
						}
						lssid = TextUtils.join(" , ", listssid);
				...
				if(intent.hasExtra("delexcludedssid")) {
					String lssid = intent.getStringExtra("delexcludedssid");
					if(lssid!=null) {
						List<String> listssid = new LinkedList<>(Arrays.asList(setting.getString("excludedSsid", "").split(" , ")));
						for(String s:lssid.split(",")){
							if(listssid.contains(s.trim())) listssid.remove(s.trim());
